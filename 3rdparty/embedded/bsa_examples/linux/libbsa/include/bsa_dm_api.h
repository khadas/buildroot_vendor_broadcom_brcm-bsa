...
typedef struct
{
    UINT8                   num_elem;   /* currently supporting MAX 3 element */
    tBSA_DM_BLE_PROP_ELEM   elem[3];
...
/* tBSA_DM_GET_DI structure */
typedef struct
{
    UINT16 vendor_id;
    UINT16 product_id;
    UINT16 version; /* binary-coded decimal, 0xJJMN for version JJ.M.N (JJ:major,M:minor,N:sub-minor) */
    BOOLEAN primary; /* is this a primary DI record ? */
    UINT16 vendor_id_source; /* 0x0001:Bluetooth SIG, 0x0002:USB Forum */
} tBSA_DM_GET_DI;

typedef tBSA_DM_GET_DI tBSA_DM_SET_DI;
...
/*******************************************************************************
 **
 ** Function         BSA_DmSetLocalDiRecord
 **
 ** Description      This function adds a DI record in device's local SDP
 **                  database.
 **
 ** Returns          tBSA_STATUS
 **
 *******************************************************************************/
tBSA_STATUS BSA_DmSetLocalDiRecord(tBSA_DM_SET_DI *p_di_record);

/*******************************************************************************
**
** Function         BSA_DmGetLocalDiRecord
**
** Description      Get the primary DI record from the local SDP database.
**
...
**
*******************************************************************************/
tBSA_STATUS BSA_DmGetLocalDiRecord(tBSA_DM_GET_DI *p_di_record);
