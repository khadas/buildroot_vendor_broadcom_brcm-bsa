 ...
 **
 *******************************************************************************/
void app_access_resp_ops(tBSA_OPS_MSG *ops_access_req_param, int accept)
{
    tBSA_STATUS status;
    tBSA_OPS_ACCESS ops_access_resp_param;
    ...
    switch (ops_access_req_param->access.oper)
    {
        case BSA_OP_OPER_PUSH :
            if (auto_access_push == FALSE)
            {
                snprintf(ops_access_resp_param.object_name,
                        sizeof(ops_access_resp_param.object_name) - 1, "/etc/bsa/%s",
        ...
        case BSA_OP_OPER_PULL :
            ...
            {
                snprintf(ops_access_resp_param.object_name,
                        sizeof(ops_access_resp_param.object_name) - 1, "/etc/bsa/test_card.vcf");
